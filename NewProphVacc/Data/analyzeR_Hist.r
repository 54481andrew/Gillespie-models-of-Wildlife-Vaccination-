        ...
        for(Yi in 1:nYVals){
            for(Xi in 1:nXVals){
		      ...
		      parmat[,FixValName2]==F2 & parmat[,FixValName3]==F3 
                ResMat[Xi,Yi] = sum(TExtMat[wifix,] > TCrit+YVal)/NTrials	                       
            }
	}
zmin = floor(10*min(ResMat))/10
zmax = 1
...
if(zmin < zmax){
            FileName = paste('Bar_',TCrit,FixValName1, F1, FixValName2, F2,FixValName3,F3,'.png',sep='')      
            png(file = paste( FigFold, '/', FileName, sep=''), height = 4, width = 5, units = 'in', res = 400)
            par(mai = c(1,1,0.25,0.25))
	    ResVec = rowMeans(ResMat)
	    barplot(ResVec)
