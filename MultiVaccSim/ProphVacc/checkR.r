SimName = "Test"
parmat = read.table(file = paste("Data/ParMat_", SimName, sep=''), header = T)
...
for(i in 1:length(parmat$NPar)){
    ...
    filename = paste("Data/", SimName, "_NPar_", i-1, sep = '')
    ...
    plot(S~time, out, xlim = c(0,365*10), ylim = c(0,30000), type = 'l', lwd = 4)
...
#########################
    ...
    par(mai = c(1,1,0.1,0.1))
    plot(S~time, out, xlim = c(5*365 - 30,5*365 + 30), ylim = c(0,10), type = 'l', lwd = 4)
    lines(Iv~time, out, col = 'green', lty = 1, lwd = 4)
    lines(Ip~time, out, col = 'red', lty = 1, lwd = 4)
    lines(V~time, out, col = 'blue', lty = 1, lwd = 4)
    lines(P~time, out, col = 'pink', lty = 1, lwd = 4)
