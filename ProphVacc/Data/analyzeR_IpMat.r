CondNonExt = TRUE
SimName = "DeerMice_Ha_2_PBShortLag_TMax1Yr"
...
IpMatFile = paste('IpMat_',SimName, sep='')
IpMat = read.table(IpMatFile, header = FALSE)
...
for(i1 in 1:nFixVals1){
    ...
    for(i3 in 1:nFixVals3){
        ResMat = matrix(ncol = nYVals, nrow = nXVals)
        for(Yi in 1:nYVals){
            for(Xi in 1:nXVals){
                XVal = XVals[Xi]
                YVal = YVals[Yi]
		...
		IpInitVal = parmat[wifix,'IpInit']
		winotzero = which(IpMat[wifix,]!=0)
if(CondNonExt){
                ResMat[Xi,Yi] = mean(unlist(IpMat[wifix,winotzero]))/IpInitVal
}else{
                ResMat[Xi,Yi] = mean(unlist(IpMat[wifix,]))/IpInitVal
}
            }
	}
zmin = 0
zmax = max(1,ceiling(max(ResMat)))
breaks = seq(zmin,zmax,by = 0.01)
nbreaks = length(breaks)
initcols = c('blue', 'cadetblue1', 'orange', 'yellow')#brewer.pal(, "Blues")
cols = colorRampPalette(initcols)(nbreaks-1)

if(zmin < zmax){
if(CondNonExt){
            FileName = paste('Ip1YrCOND_',TCrit,FixValName1, F1, FixValName2, F2,FixValName3,F3,'.png',sep='')      
}else{
            FileName = paste('Ip1Yr_',TCrit,FixValName1, F1, FixValName2, F2,FixValName3,F3,'.png',sep='')      
}
	    ...
	    image(x = XVals, y = YVals, z = ResMat, col = cols, breaks = breaks, 
