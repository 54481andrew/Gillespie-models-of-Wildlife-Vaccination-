...
##################
            ...
            FileName = paste('ExtLine_1x2_', FixValName1, F1, 
	    	     FixValName2, F2,FixValName4,F4,'.png',sep='') 
            png(file = paste( FigFold, '/', FileName, sep=''), height = 3., 
	    	     width = 5.5, units = 'in', res = 400)
            par(mai = c(0.0,0.1,0.1,0.1), omi = c(0.65, 0.6, 0, 0), mfrow = c(1,2))
...
if(zmin < zmax){
	    ...
	    matplot(NA,
		    ...
		    xlim = range(XVals), ylim = c(0,zmax), type = 'l', lwd = 2, lty = 1)
...
##       segments(x0 = min(CompValsX), y0 = MaxVal, x1 = max(CompValsX), y1 = MaxVal, col = cols[ii])
	 ...
	 y = c(min(CompValsY),max(CompValsY), max(CompValsY), min(CompValsY)), col = paste(cols[ii],"50",sep=''))

##	 polygon(x = c(min(CompValsX), min(CompValsX), max(CompValsX), max(CompValsX)), 
##	 y = CloseToMax*c(-1, 0, 0, -1) + MaxVal, border = cols[ii], lwd = 3)

       matpoints(XMax, MaxVal, pch = 8, lwd = 1, col = cols[ii])
}

	    matlines(x = XVals, y = ExtMat, col = cols, 
		    lwd = 0.75, lty = 1)
...
polygon(x = c(0,tbval,tbval, 0), y = c(-1,-1,1,1), col = colfun('gray', 75), border =NA)
	    ...
	    axis(side = 1, labels = c(0,NA,NA,NA,120,NA,NA,NA,240, NA,NA,NA,360), at = axislabs, padj = -0.8)
	    axis(side = 2, labels = i3==1, padj = 0.6)

	    mtext(text = axistext(XValName),side = 1, line = 2)
	    if(i3==1){mtext(text = "Probability of Extinction", side = 2, line = 2.25)}

	    abline(v = parmat$tb[wivals][1], lwd = 3, lty = 3)
	    if(i3==2){legend(x = 'topright', legend = paste(axistext(YValName),'=',round(YVals,1)), col = cols, lwd = 4, cex = 0.6) }

#Variable name varied across panels
legendtext = c(paste(round(1/FixVals3[i3]),'day infection'))
legend(x = -60, y = 1.075, legend = legendtext, fill = 'white', box.col = 'black', cex = 0.9)
