  ...
  for(int Par = 0; Par < NParSets; Par++) //Loop through parameters
      ...
      for(ntrial = 0; ntrial < NTrials; ntrial++)
	{
	  do {
	    ...
	    OneSim(0.0, TVaccStart + tv, true);
	  } while(t < (TVaccStart + tv)); //Throw away sims that don't last TVaccstart+tv time
	  std::cout << ntrial << "\n";
	  ...
	  for(int i5=0; i5<RhoVals.size(); i5++) //Note RhoVals
		...
		for(int i8=0; i8<dVals.size(); i8++)
		  for(int i9=0; i9<npeakVals.size(); i9++)		  
		  {
		    ...
		    d = dVals[i8];
		    tb = tbVals[i6];
		    b0 = npeak*(d*(-1 + exp(d*365)))/(exp(d*(365-tb))*(exp(d*tb) - 1));
		    gamp = gampVals[i7];
		    ParMat[i][0] = i; //Par
		    ParMat[i][1] = b0;   //b0
		    ParMat[i][2] = d; //d
