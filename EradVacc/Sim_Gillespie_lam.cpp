...
const int lamLEN = 1; double lamvals[] = {0.01};

const int NParSets = tvLEN*tbLEN*R0pLEN*RhoLEN*gampLEN*dLEN*lamLEN;
...
double ZeroMat [NParSets][NTrials];

//*******************
//CRITICAL VARIABLES
//*******************
int S, Iv, Ip, V, P, NPop, Par, IpInit, EradCount;
char FileNamePar[50];
char FileNameTExt[50];
char FileNameZero[50];
char DirName[50] = "Data/";
char FileSuffix[50], FileNameDat[50];
double b0, tb, T, Nv,tv, b, gamv, R0p, Bp, gamp, d, Event_Rate, Event_Rate_Prod, RandDeath, dTime, t, ti, TVaccStart, lam;
...
int main()
{
  ...
  strcpy(FileNameZero, DirName);
  strcat(FileNameZero, "ZeroMat");
  ...
  for(int Par = 0; Par < NParSets; Par++) //Loop through parameters
	...
	out_data << "time S Iv Ip V P N births deaths ninfv ninfp nrecv nrecp S_death Iv_death Ip_death V_death P_death svacc npopvacc totvacc totbirthson totbirthsoff\n";
      ...
      lam = ParMat[Par][12]; //Rate at which infected's are introduced
      ...
      for(ntrial = 0; ntrial < NTrials; ntrial++)
	{
	    ...
	    EradCount = 0; //Number of times pathogen goes extinct
	    OneSim(0.0, TMax, false);

	    ZeroMat[Par][ntrial] = EradCount;
	}//End loop through NTrials

      ParMat[Par][13] = (double) EradCount / (TMax - TBurnIn);
  ...
  WriteMat((double *)ZeroMat, NParSets, NTrials, FileNameZero); //Write TExtMat
  WriteMat((double *)ParMat, NParSets, NumPars, FileNamePar); //Write ParMat again
}//End Main
...
void ApplyEvent() {
  if(Event_Rate_Prod <= b) //Birth
	...
	{
	  if(Ip==1 && t > TBurnIn){EradCount++;} //Count as eradication
	  Ip--;
	  Ip_death++;
	}
  ...
  else if(Event_Rate_Prod <= b + d*NPop + (Bp*Ip*S + Bp*Ip*Iv) + gamv*Iv + gamp*Ip + lam) //Event: Spontaneous Infection!
    {Ip++; ninfp++; NPop++;}
}
...
void Initialize()
{
  ...
  lamVals.assign(lamvals, lamvals + lamLEN);
	...
	for(int i4=0; i4<IpInitVals.size(); i4++)
	  for(int i5=0; i5<RhoVals.size(); i5++) //Note RhoVals
		...
		for(int i8=0; i8<dVals.size(); i8++)
		  for(int i9=0; i9<lamVals.size(); i9++)
		    {
		      d = dVals[i8];
		      gamp = gampVals[i7];
		      ParMat[i][0] = i; //Par
		      ParMat[i][1] = 4.0;   //b0
		      ParMat[i][2] = d; //d
		      ParMat[i][3] = R0pVals[i2]*(d*365*(d + gampVals[i7]))/(4.0*tbVals[i6]); 
		      ...
		      ParMat[i][12] = lamVals[i9]; //lam val (pathogen introduction)
		      i++;
		    }
}
...
void OneSim (double StartTime, double EndTime, bool StopOnErad = false)
{
  ...
  while(t < EndTime && (Ip > 0 || !StopOnErad))
      ...
      if( t >= ti && VerboseWriteFlag) //Write old values at intervals "tick"
	{
      ...
      Event_Rate = b + d*NPop + (Bp*S*Ip + Bp*Iv*Ip) + gamv*Iv + gamp*Ip + lam;
