...
const int FreqFlagLEN = 2; int freqflagvals[]={0,1};
const int tvLEN = 1;
const int BpLEN = 3; double bpvals[] = {0.00001,0.00005,0.0001};
const int NvLEN = 1;
const int NParSets = 6;
...
//********
//CRITICAL VARIABLES
int S, Iv, Ip, V, P, NPop, Par, IpInit, FreqFlag;
...
double whichmin, unif, tmodT, denom;
...
int main()
{
  ...
  for(int Par = 0; Par < NParSets; Par++) //Loop through parameters
	...
	out_data << "time S Iv Ip V P N births deaths ninfv ninfp nrecv nrecp S_death Iv_death Ip_death V_death P_death\n";
      ...
      FreqFlag = ParMat[Par][12]; //Frequency dependent transmission
...
void ApplyEvent() {
  ...
  else if(Event_Rate_Prod <= b + d*NPop + Bp*Ip*S/denom)    //Event: Pathogen infection of S
    {S--; Ip++; ninfp++;}
  else if(Event_Rate_Prod <= b + d*NPop + (Bp*Ip*S + Bp*Ip*Iv)/denom ) //Event: Pathogen infection of V
    {Iv--; Ip++; ninfp++;}
  else if(Event_Rate_Prod <= b + d*NPop + (Bp*Ip*S + Bp*Ip*Iv)/denom + gamv*Iv) //Event: Iv Recovery
    {Iv--;V++;nrecv++;}
  else if(Event_Rate_Prod <= b + d*NPop + (Bp*Ip*S + Bp*Ip*Iv)/denom + gamv*Iv + gamp*Ip) //Event: Ip Recovery
    {Ip--; P++; nrecp++;}
}
...
void Initialize()
{
  ...
  FreqFlagVals.assign(freqflagvals, freqflagvals + FreqFlagLEN);
  ...
  for(int i1=0; i1<tvVals.size(); i1++)
	...
	for(int i4=0; i4<IpInitVals.size(); i4++)
	  for(int i5=0; i5<NvVals.size(); i5++)
	    for(int i6=0; i6<FreqFlagVals.size(); i6++)
	    {
	      ParMat[i][0] = i; //Par
	      ParMat[i][1] = 4.0;   //b0
	      ParMat[i][2] = 0.004; //d
	      ...
	      ParMat[i][12] = (double) FreqFlagVals[i6]; //Frequency dependent transmission
	      i++;
	    }
}
...
void OneSim (double StartTime, double EndTime, bool StopOnErad = false)
{
  ...
  while(t < EndTime && (Ip > 0 || !StopOnErad))
      ...
      if( t >= ti && VerboseWriteFlag) //Write old values at intervals "tick"
	{
      ...
      denom = (double) FreqFlag*(NPop-1)+1;//Frequency dependent denominator (NPop or 1)
      Event_Rate = b + d*NPop + (Bp*S*Ip + Bp*Iv*Ip)/denom + gamv*Iv + gamp*Ip;
