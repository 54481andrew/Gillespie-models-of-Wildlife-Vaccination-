...
const int lamLEN = 2; double lamvals[] = {0.01, 0.03};

const int NParSets = tvLEN*tbLEN*R0pLEN*RhoLEN*gampLEN*dLEN*lamLEN;//52*2*5*3*3*2;
...
void OneSim (double StartTime, double EndTime, bool StopOnErad = false)
{
  ...
  while(t < EndTime && (Ip > 0 || !StopOnErad))
      ...
      if( t >= ti && VerboseWriteFlag) //Write old values at intervals "tick"
	{
      ...
      Event_Rate = b + d*NPop + (Bp*S*Ip + Bp*Iv*Ip)/NPop + gamv*Iv + gamp*Ip + lam;
