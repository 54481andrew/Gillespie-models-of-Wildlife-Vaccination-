...
char SimName[50] = "Test_DeerMice_Base";
...
int main()
{
  ...
  for(int Par = 0; Par < NParSets; Par++) //Loop through parameters
	...
	out_data << "time S Iv Ip V P N births deaths ninfv ninfp nrecv nrecp S_death Iv_death Ip_death V_death P_death svacc npopvacc nvacc\n";
      ...
      TVaccStart = ParMat[Par][11]; // Time at which vaccination starts
...
void Initialize()
{
  //tvVals.assign(tvvals, tvvals + tvLEN); //Run a discrete set of tvvals
   tvVals = Seq(85.0, 95.0, tvLEN); //Run a Seq() of values
